...
**Phase 2（充実）— 問題数の目標（800問）を達成。**

- CIでビルド・ユニットテスト88件・UIテスト2件がすべて通り、
  シミュレータ上で各画面が描画されることをスクリーンショットで確認済み
- TestFlightへ配信済み（実機での動作確認が可能）
- **問題800問**を同梱。分野比率は本試験と一致（35 : 20 : 45）、全23中分類に20問以上。
  追加した500問はシラバスVer.6.5（2026年1月）準拠で、中小受託取引適正化法などの新しい法令名に対応

**次にやること: 問題内容の人手レビュー**（下記「残っている作業」を参照）。
...
無料で配布し、買い切りのApp内課金（¥300）で「標準問題（difficulty=2）」と「応用問題（difficulty=3）」を出題対象に加える形にしている。
**広告とサブスクリプションは実装しない。**
...
```bash
python scripts/fix_logo_badge.py   # 右上バッジの文言を描く（変えるときだけ）
python scripts/make_app_icon.py    # アイコンとアプリ内ロゴを書き出す
```

| ファイル | 役割 |
| --- | --- |
| `docs/assets/itpassport-logo-original.png` | 受け取った元画像。バッジに「NO GLYPH」の豆腐が焼き込まれた状態 |
| `docs/assets/itpassport-logo-source.png` | 上のバッジを「試験対策」に描き直したもの。**アイコン生成の入力はこちら** |

`make_app_icon.py` は App Store の要件（1024x1024・アルファなし・角丸を焼き込まない）を
満たすよう自動で整える。具体的には次の3つを行う。

- **白い余白と境界の除去** — 受け取った画像は絵柄が版面の中央になく、左に27px・下に16pxの
  白帯があった。そのままだとAppleのマスク後に左下だけ白い縁が出る。
  白との境目に残る中間色の1pxも落とす（残すと余白を作るときに引き伸ばされ、明るい帯になる）
- **正方形への切り出し** — 比率を変えて引き伸ばすと文字が歪むため、中央から切り出す
- **マスク内への収まり調整** — 角に近い絵柄がAppleの角丸マスクで切られないか判定し、
  掛かる場合だけ縮めて余白を足す

バッジの文言を変えるときは `fix_logo_badge.py` の `BADGE_TEXT` を書き換えて両方を実行する。

## 残っている作業

| 項目 | 内容 |
| --- | --- |
| **問題内容の人手レビュー** | **800問すべて未レビュー。**下記のとおり最優先 |
| 公開URL | `AppConfig.privacyPolicyURL` / `supportURL` は仮のURL。公開前に実在のページへ差し替える |
| App内課金の登録 | `com.eitango.itpassport.unlock.advanced` が未登録のため購入画面に価格が出ない（`docs/testflight-setup.md` §7） |
| シラバス確認 | 追加分はVer.6.5で作成。最初の300問（Ver.6.3で作成）はVer.6.5の用語例と突き合わせること |
| 模擬試験モード | Phase 2（設計仕様書 §11） |

### 問題内容のレビューが必要な理由

同梱の800問はAIが生成したもので、機械検証（形式・ID重複・問題文の重複・分野と中分類の
対応・4つの解説の有無と書き出し・文字数）は通っているが、**書かれている内容が正しいか
どうかは検証されていない**。

国家試験の対策アプリで解説が誤っていると、利用者は誤った知識を「正解」として覚える。
公開前に設計仕様書 §9.3 のチェックリストで全問を確認すること。特に次の3点は
機械では検出できない。

- **法規**（LEGAL 50問）— 改正が速い領域。現行法との一致を確認する
- **計算問題**（THEORY / CORP / SVCMGT / NW などに点在）— 検算する
- **正解の一意性** — 「最も適切なもの」形式で、他の選択肢が本当に劣るかを確認する
